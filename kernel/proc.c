...
found:
  ...
  // Allocate a trapframe page. 中断异常页
...
freeproc(struct proc *p)
{
  ...
  {
    // free the kernel stack in RAM.
    uvmunmap(p->kernel_pagetable, p->kstack, 1, 1);
    p->kstack = 0;
    // free page
    freewalk_2(p->kernel_pagetable);
  }
  p->pagetable = 0;
  p->sz = 0;
  p->pid = 0;
...
void freewalk_2(pagetable_t kernelpt)
{
  for (int i = 0; i < 512; i++)
  {
